...
export function ExperienceSection() {
  ...
  return (
    <section 
      ref={containerRef}
      id="experience" 
      className={`relative w-full min-h-screen py-32 transition-colors duration-700 ${isDark ? "bg-[#0A0A0F]" : "bg-[#f5f5f7]"}`}
    >
      <div className="max-w-5xl mx-auto px-6 relative z-10">
        ...
        >
          <p className="font-mono-brand text-xs text-[#7c6bff] uppercase tracking-widest mb-3">02 — The Journey</p>
          <h2 className={`text-4xl md:text-5xl font-extrabold tracking-tight ${isDark ? "text-white" : "text-[#1d1d1f]"}`}>
            Experience & <span className="text-[#34d399]">Leadership</span>.
          </h2>
        </motion.div>
        ...
        <div className="relative">
          ...
          {/* Animated Glow Line */}
          <motion.div 
            className="absolute left-6 md:left-1/2 top-0 bottom-0 w-[4px] -translate-x-1/2 origin-top rounded-full bg-gradient-to-b from-[#7c6bff] via-[#34d399] to-transparent shadow-[0_0_20px_rgba(124,107,255,0.6)]"
            style={{ scaleY }}
          />
            ...
            {experiences.map((exp, index) => {
              ...
              return (
                <div key={exp.id} className={`flex flex-col md:flex-row items-center justify-between w-full relative ${isEven ? 'md:flex-row-reverse' : ''}`}>
                  ...
                  {/* Timeline Dot */}
                  <div className="absolute left-6 md:left-1/2 top-1/2 -translate-y-1/2 -translate-x-1/2 w-4 h-4 rounded-full bg-[#0A0A0F] border-2 border-[#34d399] shadow-[0_0_15px_rgba(52,211,153,0.8)] z-10 hidden md:block" />
                  
                  {/* Mobile Dot */}
                  <div className="absolute left-6 top-8 -translate-x-1/2 w-4 h-4 rounded-full bg-[#0A0A0F] border-2 border-[#34d399] shadow-[0_0_15px_rgba(52,211,153,0.8)] z-10 md:hidden" />
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex flex-col mb-4 relative z-10">
                        <span className={`font-mono-brand text-[10px] uppercase tracking-widest mb-2 ${isDark ? "text-[#34d399]" : "text-[#059669]"}`}>{exp.date}</span>
